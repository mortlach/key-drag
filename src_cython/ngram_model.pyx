...
class NGramModel:
    ...
    __w3_sd = {1: 2.21654,2: 2.21654,3: 2.21654,4: 2.21654,5: 2.21654,6: 2.21654, 7: 2.03109, 8: 1.89282, 9 : 1.77089, 10: 1.67998, 11: 1.59448, 12: 1.53094}
    __w4_sd = {1: 3.0063, 6: 2.0063, 3: 3.0063, 4: 3.0063, 5: 3.0063, 6: 3.0063,6: 3.0063, 7: 2.73505, 8: 2.54176, 9 : 2.35905, 10: 2.23328, 11: 2.10699, 12: 2.02206}
    ...
    def load_raw_tm_data_w3(self):
        if not NGramModel.__w3_loaded:
            ...
            with open('./data/tm_wli_models3p.csv', newline='\n') as csvfile:
                NGramModel.__tm_rwli_3 = {literal_eval(','.join(row[:-1])) : float(row[-1]) for row in csv.reader(csvfile, delimiter=',', quotechar='|')}
    ...
    def load_raw_tm_data_w4(self):
        if not NGramModel.__w4_loaded:
            ...
            with open('./data/tm_wli_models4p.csv', newline='\n') as csvfile:
                    NGramModel.__tm_rwli_4 = {literal_eval(','.join(row[:-1])) : float(row[-1]) for row in csv.reader(csvfile, delimiter=',', quotechar='|')}
    ...
    def get_logprob_wlir_data_w3(self, runes, wli, subtract_mean=True):
        return [self.__get_tm_rwli_3(tuple([wli[i][1], wli[i][0], runes[i], runes[i + 1],
                                     runes[i + 2]])) - NGramModel.__w3_mean if subtract_mean else self.__get_tm_rwli_3(
    ...
    def get_logprob_wlir_data_w4(self, runes, wli, subtract_mean=True):
        return [self.__get_tm_rwli_4(tuple([wli[i][1], wli[i][0], runes[i], runes[i + 1], runes[i + 2],
                                     runes[i + 3]])) - NGramModel.__w4_mean if subtract_mean else self.__get_tm_rwli_4(
    ...
    def get_distance_from_mean_w4(self,rune,wli):
        ...
        mean_dif = [
            sum(rw4) / (len(rw4) * NGramModel.__w3_sd.get(len(rw4), NGramModel.__w3_sd[12])),
        ]
        return mean_dif
